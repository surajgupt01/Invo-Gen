...
export default function InvoicePreview() {
  ...
  // Formatting configuration based on static sample data
      ...
      <div className="w-full">
        <div className="flex justify-between items-start border-b border-gray-300 p-1 h-auto">
          <div className="flex justify-start items-start gap-2">
            <div className={`${!OwnerDetails.companyLogo ? "border" : ""} border-gray-400 w-24 h-24 flex items-center justify-center`}>
              {OwnerDetails.companyLogo ? (
                <Image alt="logo" src={OwnerDetails.companyLogo} width={80} height={70} className="object-contain" />
              ) : (
                <span className="text-xl font-bold text-gray-400">{OwnerDetails.CompanyName?.charAt(0) || "N"}</span>
              )}
            </div>
        ...
        <div className="flex flex-col">
          <div className="grid grid-cols-2 gap-2 w-full text-[10px] mt-4">
            <div className="bg-gray-50 p-2 rounded-lg">
              ...
              <p className="text-gray-600 whitespace-pre-line">{Details.CustomerAddress}</p>
            </div>
              ...
              <tbody>
                {Items.map((item, idx) => (
                  <tr key={idx} className="border-b last:border-none text-[8px]">
                    ...
                    <td className="px-4 py-2 text-center text-gray-500">{item.unit || "pcs"}</td>
                    <td className="px-4 py-2 text-center">{item.qty}</td>
                    <td className="px-4 py-2 text-right">{sym}{fmt(item.rate)}</td>
                    <td className="px-4 py-2 text-center text-gray-500">{item.discount || 0}%</td>
